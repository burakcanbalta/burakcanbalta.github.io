...
## 11. Sonuç ve Kök Neden Analizi

| Bulgu | Kök Neden | Etki |
|-------|-----------|------|
| SMB paylaşımlarında hassas not dosyaları | `Notes` ve `Trainees` gibi paylaşımların gereğinden fazla açık bırakılması | Zayıf şifre politikası ve dahili notların dışarı sızması |
| `trainee:trainee` | Kullanıcı adı ile aynı, tahmin edilebilir şifre | Domain'e authenticated kullanıcı olarak ilk erişim |
| Şifresiz `BANKING$` bilgisayar hesabı | Pre-created makine hesabının hiç kullanılmaması/temizlenmemesi, şifresinin serbestçe değiştirilebilir olması | Ek bir domain kimliği (bilgisayar hesabı) ele geçirildi |
| ADCS `RetroClients` şablonunda ESC1 | Şablonun "enrollee supplies subject" izniyle birlikte geniş bir gruba (Domain Computers) enroll hakkı vermesi | Herhangi bir domain bilgisayar hesabının Administrator adına sertifika alıp tam yetki kazanabilmesi |

**Kısa remediation notları:**

1. SMB paylaşımlarında "herkese açık okuma" yerine ihtiyaç bazlı, en az yetki prensibiyle erişim tanımlanmalı; hassas notlar paylaşım üzerinde bırakılmamalı.
2. Şifre politikası kullanıcı adıyla aynı/benzer şifrelere izin vermemeli.
3. Kullanılmayan/pre-created bilgisayar hesapları düzenli olarak temizlenmeli veya devre dışı bırakılmalı.
4. ADCS şablonlarında "Enrollee Supplies Subject" özelliği, sadece gerçekten gerekli ve dar bir gruba (asla Domain Computers/Domain Users gibi geniş gruplara değil) tanınmalı — bu, ESC1'in klasik önleme yöntemidir.

---

## 12. Zaman Çizelgesi (Timeline)

1. Nmap taraması → AD Domain Controller (retro.vl) tespiti
2. SMB null session ile paylaşımlar listelendi, `Trainees` paylaşımından `Important.txt` alındı
3. Not, ortak/zayıf şifre kullanıldığına işaret etti
4. `guest` ile RID brute-force yapılarak kullanıcı listesi çıkarıldı
5. `trainee:trainee` denendi, giriş başarılı oldu
6. `Notes` paylaşımından `user.txt` ve `ToDo.txt` alındı; ToDo, pre-created bir bilgisayar hesabına işaret etti
7. `BANKING$` hesabının şifresi SAMR protokolü üzerinden değiştirildi
8. `certipy-ad find -vulnerable` ile ADCS'de ESC1 zafiyetli `RetroClients` şablonu bulundu
9. `BANKING$` kimliğiyle Administrator adına sertifika talep edildi ve onaylandı
10. Sertifika ile Kerberos auth yapıldı, Administrator NT hash'i elde edildi
11. Pass-the-hash ile `impacket-psexec` üzerinden Administrator shell alındı
12. `root.txt` okunarak kutu tamamlandı
